...
class PPONetwork(object):
    ...
    def __init__(self, sess, obs_dim, act_dim, name):
        ...
        with tf.variable_scope(name):
            ...
            h1 = activation(fc(X,'fc1', nh=512*2, init_scale=np.sqrt(2)))
            h2 = activation(fc(h1,'fc2', nh=256*2, init_scale=np.sqrt(2)))
            pi = fc(h2,'pi', act_dim, init_scale = 0.01)
            #value function - share layer h1
            h3 = activation(fc(h1,'fc3', nh=256*2, init_scale=np.sqrt(2)))
            vf = fc(h3, 'vf', 1)[:,0]
