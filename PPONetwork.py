...
class PPONetwork(object):
    ...
    def __init__(self, sess, obs_dim, act_dim, name):
        ...
        def sample():
            u = tf.random.uniform(tf.shape(input=availPi))
            return tf.argmax(input=availPi - tf.math.log(-tf.math.log(u)), axis=-1)
        
        a0 = sample()
        el0in = tf.exp(availPi - tf.reduce_max(input_tensor=availPi, axis=-1, keepdims=True))
        z0in = tf.reduce_sum(input_tensor=el0in, axis=-1, keepdims = True)
        p0in = el0in / z0in
        onehot = tf.one_hot(a0, availPi.get_shape().as_list()[-1])
        neglogpac = -tf.math.log(tf.reduce_sum(input_tensor=tf.multiply(p0in, onehot), axis=-1))
...
class PPOModel(object):
    ...
    def __init__(self, sess, network, inpDim, actDim, ent_coef, vf_coef, max_grad_norm):
        ...
        l0 = network.availPi - tf.reduce_max(input_tensor=network.availPi, axis=-1, keepdims=True)
        el0 = tf.exp(l0)
        z0 = tf.reduce_sum(input_tensor=el0, axis=-1, keepdims=True)
        p0 = el0 / z0
        entropy = -tf.reduce_sum(input_tensor=(p0+1e-8) * tf.math.log(p0+1e-8), axis=-1)
        oneHotActions = tf.one_hot(ACTIONS, network.pi.get_shape().as_list()[-1])
        neglogpac = -tf.math.log(tf.reduce_sum(input_tensor=tf.multiply(p0, oneHotActions), axis=-1))
        ...
        #entropy loss
        entropyLoss = tf.reduce_mean(input_tensor=entropy)
        #value loss
        v_pred = network.vf
        ...
        vf_loss = 0.5 * tf.reduce_mean(input_tensor=tf.maximum(vf_losses1, vf_losses2))
        ...
        pg_loss = tf.reduce_mean(input_tensor=tf.maximum(pg_losses1, pg_losses2))
        #total loss
